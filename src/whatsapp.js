...
async function chamarIA(loja, numeroCliente, mensagem, tipo, imgB64) {
  ...
  console.log(`[HTTP-REQUEST] Chamando AI Engine em: ${PYTHON_URL}/chat`)
...
class WaInstance {
  ...
  async conectar(numero = null) {
    if (this.sock) {
      try { this.sock.ev.removeAllListeners(); this.sock.terminate() } catch (e) {}
      this.sock = null
    }
  ...
  async _processar(msg) {
    ...
    try {
      ...
      console.error(`[DIAGNOSTICO][${this.lojaId}] Falha na AI Engine:`, err.message)
...
class Manager {
  ...
  async desconectar(lojaId, deletarSessao = true) {
    ...
    if (inst) await inst.desconectar(deletarSessao)
  }
  stateOf(lojaId) { return this._map.get(lojaId)?.state() ?? { lojaId, status: 'aguardando' } }
  async reconectarSessoes() {
    ...
    for (const dir of dirs) {
      ...
      this._getOrCreate(lojaId).conectar(null).catch(() => {})
    }
  }
}
